...
OUT_NAME = fq.MACHINE+str(fq.SITES)+str(fq.TOTAL_SZ) # output file name
OUT_LOG_NAME = "out_"+fq.NAME+".txt"            # filename for output logging
...
for JEXCH1 in fq.STEPS:
    ...
    if fq.MACHINE == "RBM":
        ...
        machine_1 = GCNN_my(symmetries=g.automorphisms(), dtype=fq.DTYPE, layers=1, features=fq.ALPHA, characters=characters_1, output_activation=nk.nn.log_cosh, use_bias=True, use_visible_bias=True)
        machine_2 = GCNN_my(symmetries=g.automorphisms(), dtype=fq.DTYPE, layers=1, features=fq.ALPHA, characters=characters_2, output_activation=nk.nn.log_cosh, use_bias=True, use_visible_bias=True)
    else:
    ...
    with open("out_err_"+fq.NAME+".txt", "a") as file_mag:
        if JEXCH1 == fq.STEPS[0]:
        ...
        if no_of_runs == 2:
            ...
            print("{:5.2f}  {:10.5f}     {:10.5f} {:10.5f} {:10.5f} {:10.5f} ".format(JEXCH1, exact_ground_energy, gs.energy.mean.real, gs.energy.error_of_mean, np.mean(energy_convergence[0][-50:]), np.std(energy_convergence[0][-50:])), file=file_mag)
