...
for m in range(0,30):
    for j in range(no_repeats):
        ...
        if exact_ground_energy != 0 and fq.VERBOSE == True:
            print("i =",i,"j =",j,"; Expected exact energy:", exact_ground_energy)
        for i,gs in enumerate([gs_1,gs_2][use_2:use_2+no_of_runs]):
            start = time.time()
            gs.run(out=OUT_NAME+"_"+str(round(fq.JEXCH1,2))+"_"+str(i), n_iter=int(fq.NUM_ITER),show_progress=fq.VERBOSE)
            end = time.time()
            if m == 0:
        ...
        for i in range(no_of_runs):
            data.append(json.load(open(OUT_NAME+"_"+str(round(fq.JEXCH1,2))+"_"+str(i)+".log")))
